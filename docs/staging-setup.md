...
このリポジトリは `Next.js + Supabase` 構成なので、いちばん早いステージングは `Vercel` にステージング用プロジェクトを作る方法です。

## おすすめ構成

- アプリ: `Vercel`
- DB / Auth / Storage: 既存 `Supabase`
- ステージングURL: `https://stg.vnd.kuridas-os.jp`
...
モバイルオーダー確認を急ぐなら、まずはベンダー用の `stg.vnd.kuridas-os.jp` だけで十分です。
...
5. `codex/mobile-order-foundation` をステージング用プロジェクトの Production Branch にする
6. ステージング用のカスタムドメイン `stg.vnd.kuridas-os.jp` を紐づける
...
```env
APP_URL=https://stg.vnd.kuridas-os.jp
...
VENDOR_APP_HOST=stg.vnd.kuridas-os.jp
ORGANIZER_APP_HOST=stg.org.kuridas-os.jp
...
- ベンダー: `stg.vnd.kuridas-os.jp`
- 主催者: `stg.org.kuridas-os.jp`

今回はモバイルオーダー確認が主目的なので、先に `stg.vnd.kuridas-os.jp` だけでも問題ありません。
...
```text
https://stg.vnd.kuridas-os.jp/liff/mobile-order
```
...
```text
https://stg.vnd.kuridas-os.jp/liff/mobile-order?token=xxxxxxxxxxxxxxxx
```
...
3. 固定注文URLが `https://stg.vnd.kuridas-os.jp/order/...` になる
4. LIFF入口URLを LINE で開ける
5. 注文時に `customer_line_user_id` が保存される
6. 注文完了通知 / 調理開始通知 / 完成通知が届く
7. クレジットカード決済後に注文が確定する
...
- `STRIPE_SECRET_KEY` と `STRIPE_WEBHOOK_SECRET` も Vercel の Production に必ず登録してください
- Stripe の webhook 送信先は `https://stg.vnd.kuridas-os.jp/api/stripe/webhook` です
...
4. `stg.vnd.kuridas-os.jp` を割り当て
5. LIFF Endpoint URL を `https://stg.vnd.kuridas-os.jp/liff/mobile-order` に変更
