import { ensureVendorStoreResources } from '@/lib/mobile-order'
import type {
  MobileOrderOptionChoiceRow,
  MobileOrderOptionGroupRow,
  MobileOrderProductRow,
  VendorMobileOrderOptionGroup,
  VendorMobileOrderOptionGroupMutationPayload,
  VendorMobileOrderOptionsPayload,
} from '@/types/api-payloads'

type ChoiceInput = {
  name: string
  price_delta: number
  sort_order: number
  is_active: boolean
}

function normalizeBoolean(value: unknown, fallback = false) {
  return typeof value === 'boolean' ? value : fallback
}

async function loadVendorBusinessName(supabase: any, userId: string) {
  const { data } = await (supabase as any)
    .from('vendor_profiles')
    .select('business_name')
    .eq('user_id', userId)
    .maybeSingle()

  return data?.business_name ?? null
}

export function buildOptionGroupsPayload(args: {
  groups: MobileOrderOptionGroupRow[]
  choices: MobileOrderOptionChoiceRow[]
  links: Array<{ product_id: string; option_group_id: string }>
}): VendorMobileOrderOptionGroup[] {
  const choicesByGroup = new Map<string, MobileOrderOptionChoiceRow[]>()
  for (const choice of args.choices) {
    const current = choicesByGroup.get(choice.group_id) ?? []
    current.push(choice)
    choicesByGroup.set(choice.group_id, current)
  }

  const linkedProductIdsByGroup = new Map<string, string[]>()
  for (const link of args.links) {
    const current = linkedProductIdsByGroup.get(link.option_group_id) ?? []
    current.push(link.product_id)
    linkedProductIdsByGroup.set(link.option_group_id, current)
  }

  return args.groups.map((group) => ({
    ...group,
    choices: (choicesByGroup.get(group.id) ?? []).sort((a, b) => a.sort_order - b.sort_order),
    linked_product_ids: linkedProductIdsByGroup.get(group.id) ?? [],
  }))
}

export async function loadOptionAdminData(supabase: any, storeId: string) {
  const [
    { data: products, error: productsError },
    { data: groups, error: groupsError },
    { data: choices, error: choicesError },
    { data: links, error: linksError },
  ] = await Promise.all([
    supabase.from('mobile_order_products').select('*').eq('store_id', storeId).order('sort_order', { ascending: true }).order('created_at', { ascending: true }),
    supabase.from('mobile_order_option_groups').select('*').eq('store_id', storeId).order('sort_order', { ascending: true }).order('created_at', { ascending: true }),
    supabase
      .from('mobile_order_option_choices')
      .select('*, mobile_order_option_groups!inner(store_id)')
      .eq('mobile_order_option_groups.store_id', storeId)
      .order('sort_order', { ascending: true }),
    supabase
      .from('mobile_order_product_option_groups')
      .select('product_id, option_group_id, mobile_order_products!inner(store_id)')
      .eq('mobile_order_products.store_id', storeId),
  ])

  if (productsError) throw new Error(productsError.message)
  if (groupsError) throw new Error(groupsError.message)
  if (choicesError) throw new Error(choicesError.message)
  if (linksError) throw new Error(linksError.message)

  return {
    products: (products ?? []) as MobileOrderProductRow[],
    optionGroups: buildOptionGroupsPayload({
      groups: (groups ?? []) as MobileOrderOptionGroupRow[],
      choices: ((choices ?? []) as Array<MobileOrderOptionChoiceRow & { mobile_order_option_groups: { store_id: string } }>).map(
        ({ mobile_order_option_groups: _ignored, ...choice }) => choice
      ),
      links: ((links ?? []) as Array<{ product_id: string; option_group_id: string; mobile_order_products: { store_id: string } }>).map(
        ({ mobile_order_products: _ignored, ...link }) => link
      ),
    }),
  }
}

export async function getVendorMobileOrderOptionsPayload(supabase: any, user: { id: string }): Promise<VendorMobileOrderOptionsPayload> {
  const businessName = await loadVendorBusinessName(supabase, user.id)
  const { store } = await ensureVendorStoreResources(supabase, user, { businessName })
  const payloadBase = await loadOptionAdminData(supabase, store.id)
  return {
    store,
    products: payloadBase.products,
    optionGroups: payloadBase.optionGroups,
  }
}

export function parseOptionGroupInput(body: any, mode: 'create' | 'update') {
  const name = typeof body.name === 'string' ? body.name.trim() : ''
  const selectionType = typeof body.selection_type === 'string' ? body.selection_type.trim() : ''
  const isRequired = normalizeBoolean(body.is_required, false)
  const minSelect = body.min_select == null || body.min_select === '' ? null : Number(body.min_select)
  const maxSelect = body.max_select == null || body.max_select === '' ? null : Number(body.max_select)
  const sortOrder = Number.isFinite(Number(body.sort_order)) ? Number(body.sort_order) : 0
  const linkedProductIds = Array.isArray(body.linked_product_ids)
    ? body.linked_product_ids.map((value: unknown) => String(value))
    : mode === 'create'
      ? []
      : null
  const choicesInput = Array.isArray(body.choices) ? body.choices : mode === 'create' ? [] : null

  if (!name) throw new Error('オプショングループ名は必須です')
  if (!['single', 'multiple'].includes(selectionType)) throw new Error('選択方式が不正です')
  if (choicesInput && choicesInput.length === 0) throw new Error('選択肢を1件以上入力してください')

  const normalizedChoices =
    choicesInput?.map((choice: any, index: number) => ({
      name: String(choice?.name ?? '').trim(),
      price_delta: Number(choice?.price_delta ?? 0),
      sort_order: Number.isFinite(Number(choice?.sort_order)) ? Number(choice.sort_order) : index,
      is_active: normalizeBoolean(choice?.is_active, true),
    })) ?? null

  if (normalizedChoices?.some((choice: ChoiceInput) => !choice.name)) {
    throw new Error('選択肢名は必須です')
  }

  return {
    name,
    selectionType,
    isRequired,
    minSelect,
    maxSelect,
    sortOrder,
    linkedProductIds,
    normalizedChoices,
  }
}

export async function loadVendorOwnedOptionGroup(supabase: any, userId: string, groupId: string) {
  const { data: currentGroup, error: currentGroupError } = await (supabase as any)
    .from('mobile_order_option_groups')
    .select('*, vendor_stores!inner(vendor_user_id)')
    .eq('id', groupId)
    .eq('vendor_stores.vendor_user_id', userId)
    .single()

  if (currentGroupError || !currentGroup) {
    throw new Error('対象のオプショングループが見つかりません')
  }

  return currentGroup
}

export async function createVendorOptionGroup(
  supabase: any,
  user: { id: string },
  input: ReturnType<typeof parseOptionGroupInput>
): Promise<VendorMobileOrderOptionGroupMutationPayload> {
  const businessName = await loadVendorBusinessName(supabase, user.id)
  const { store } = await ensureVendorStoreResources(supabase, user, { businessName })

  const { data: group, error: groupError } = await (supabase as any)
    .from('mobile_order_option_groups')
    .insert([
      {
        store_id: store.id,
        name: input.name,
        is_required: input.isRequired,
        selection_type: input.selectionType,
        min_select: input.minSelect,
        max_select: input.maxSelect,
        sort_order: input.sortOrder,
      },
    ])
    .select('*')
    .single()

  if (groupError) throw new Error(groupError.message)

  const { data: choices, error: choicesError } = await (supabase as any)
    .from('mobile_order_option_choices')
    .insert(
      (input.normalizedChoices ?? []).map((choice: ChoiceInput) => ({
        group_id: group.id,
        name: choice.name,
        price_delta: choice.price_delta,
        sort_order: choice.sort_order,
        is_active: choice.is_active,
      }))
    )
    .select('*')

  if (choicesError) throw new Error(choicesError.message)

  if ((input.linkedProductIds ?? []).length > 0) {
    const { error: linkError } = await (supabase as any)
      .from('mobile_order_product_option_groups')
      .insert(
        (input.linkedProductIds ?? []).map((productId: string, index: number) => ({
          product_id: productId,
          option_group_id: group.id,
          sort_order: index,
        }))
      )
    if (linkError) throw new Error(linkError.message)
  }

  return {
    ...(group as MobileOrderOptionGroupRow),
    choices: (choices ?? []) as MobileOrderOptionChoiceRow[],
    linked_product_ids: input.linkedProductIds ?? [],
  }
}

export async function updateVendorOptionGroup(
  supabase: any,
  groupId: string,
  input: ReturnType<typeof parseOptionGroupInput>
): Promise<VendorMobileOrderOptionGroupMutationPayload> {
  const { data: updatedGroup, error: updateError } = await (supabase as any)
    .from('mobile_order_option_groups')
    .update({
      name: input.name,
      selection_type: input.selectionType,
      is_required: input.isRequired,
      min_select: input.minSelect,
      max_select: input.maxSelect,
      sort_order: input.sortOrder,
    })
    .eq('id', groupId)
    .select('*')
    .single()

  if (updateError) throw new Error(updateError.message)

  if (input.normalizedChoices) {
    const { error: deleteChoicesError } = await (supabase as any)
      .from('mobile_order_option_choices')
      .delete()
      .eq('group_id', groupId)
    if (deleteChoicesError) throw new Error(deleteChoicesError.message)

    const { error: insertChoicesError } = await (supabase as any)
      .from('mobile_order_option_choices')
      .insert(
        input.normalizedChoices.map((choice: ChoiceInput) => ({
          group_id: groupId,
          name: choice.name,
          price_delta: choice.price_delta,
          sort_order: choice.sort_order,
          is_active: choice.is_active,
        }))
      )
    if (insertChoicesError) throw new Error(insertChoicesError.message)
  }

  if (input.linkedProductIds) {
    const { error: deleteLinksError } = await (supabase as any)
      .from('mobile_order_product_option_groups')
      .delete()
      .eq('option_group_id', groupId)
    if (deleteLinksError) throw new Error(deleteLinksError.message)

    if (input.linkedProductIds.length > 0) {
      const { error: insertLinksError } = await (supabase as any)
        .from('mobile_order_product_option_groups')
        .insert(
          input.linkedProductIds.map((productId: string, index: number) => ({
            product_id: productId,
            option_group_id: groupId,
            sort_order: index,
          }))
        )
      if (insertLinksError) throw new Error(insertLinksError.message)
    }
  }

  const [{ data: finalChoices, error: finalChoicesError }, { data: finalLinks, error: finalLinksError }] = await Promise.all([
    (supabase as any).from('mobile_order_option_choices').select('*').eq('group_id', groupId).order('sort_order', { ascending: true }),
    (supabase as any).from('mobile_order_product_option_groups').select('product_id').eq('option_group_id', groupId).order('sort_order', { ascending: true }),
  ])
  if (finalChoicesError) throw new Error(finalChoicesError.message)
  if (finalLinksError) throw new Error(finalLinksError.message)

  return {
    ...(updatedGroup as MobileOrderOptionGroupRow),
    choices: (finalChoices ?? []) as MobileOrderOptionChoiceRow[],
    linked_product_ids: ((finalLinks ?? []) as Array<{ product_id: string }>).map((link) => link.product_id),
  }
}
