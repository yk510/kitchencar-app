...
function resolveSchedules(schedules: StoreOrderScheduleRow[]) {
  ...
  const nextSchedule =
    sorted.find((schedule) => schedule.status !== 'cancelled' && new Date(schedule.opens_at).getTime() > now) ?? null
...
export default async function OrderPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [{ data: schedules }, { data: products }, { data: optionGroups }, { data: optionChoices }, { data: links }] =
    await Promise.all([
      (supabase as any)
